...
/* Action list type. */
typedef const unsigned long *dasm_ActList;

/* Per-section structure. */
typedef struct dasm_Section {
  long *rbuf;		/* Biased buffer pointer (negative section bias). */
  long *buf;		/* True buffer pointer. */
...
void dasm_put(Dst_DECL, int start, ...)
{
  ...
  if (pos >= sec->epos) {
    DASM_M_GROW(Dst, long, sec->buf, sec->bsize,
      sec->bsize + 2*DASM_MAXSECPOS*sizeof(long));
    sec->rbuf = sec->buf - DASM_POS2BIAS(pos);
    sec->epos = (int)sec->bsize/sizeof(long) - DASM_MAXSECPOS+DASM_POS2BIAS(pos);
  }
...
int dasm_encode(Dst_DECL, void *buffer)
{
  ...
  unsigned long *cp = (unsigned long *)buffer;
  int secnum;
